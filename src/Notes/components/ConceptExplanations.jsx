...
const ConceptExplanations = ({ concepts, editorRef }) => {
  const [expandedComments, setExpandedComments] = useState(new Set());
  const [conceptPositions, setConceptPositions] = useState(new Map());
  const [adjustedPositions, setAdjustedPositions] = useState(new Map());
  ...
  useEffect(() => {
    ...
    const handleConceptClick = (e) => {
      const conceptMark = e.target.closest('.concept-mark');
      if (conceptMark) {
    ...
    const handleDocumentClick = (e) => {
      if (!explanationsRef.current?.contains(e.target) && 
          !e.target.closest('.concept-mark')) {
