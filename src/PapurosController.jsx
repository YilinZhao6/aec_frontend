  ...
  const startEventSource = (query, bookIds, enableWebSearch, newConversationId) => {
    ...
    eventSource.onmessage = (event) => {
      ...
      if (message.includes(`Starting Google search and content collection for user '${userId}' in conversation '${newConversationId}'`)) {
        ...
        console.log('Switching to section writing view');
        
        fetch(`https://backend-ai-cloud-explains.onrender.com/check_outline?user_id=${userId}&conversation_id=${newConversationId}`)
          .then(response => response.json())
          .then(data => {
            if (data.exists && data.sections) {
              console.log('Outline fetched successfully:', data.sections);
              setOutlineSections(data.sections);
              setView('section_writing');
            } else {
              console.error('Outline not found or no sections available:', data);
              alert('Error: No outline found for this conversation.');
            }
          })
          .catch(error => {
            console.error('Error fetching outline:', error);
            alert('Error fetching outline. Please try again later.');
          });
      }
      else if (message.includes(`Article generation completed successfully for user '${userId}' in conversation '${newConversationId}'`)) {
        console.log('Generation complete, fetching article');
        eventSource.close();
      
        fetch(`https://backend-ai-cloud-explains.onrender.com/article?user_id=${userId}&conversation_id=${newConversationId}`)
          .then((response) => {
            if (!response.ok) {
